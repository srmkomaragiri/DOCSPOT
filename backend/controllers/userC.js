...
const registerDoctorController = async (req, res) => {
  try {
    ...
    const existingDoctor = await docSchema.findOne({ userId: userId });
    if (existingDoctor) {
    ...
    // Add notification to admin about new doctor application
    const adminUser = await userSchema.findOne({ isAdmin: true });
    if (adminUser) {
      const notification = adminUser.notification || [];
      notification.push({
        type: "apply-doctor-request",
        message: `${doctor.fullName} has applied for doctor account`,
        data: {
    ...
    return res.status(201).send({
      success: true,
      message: "Doctor application submitted successfully. Please wait for approval."
    });
    ...
    return res.status(500).send({
      success: false,
      message: "Something went wrong while applying for doctor account"
    });
  }
};
